...
//		exec.runGameTimed(new Human(),new AttractRepelGhosts(true),true);
		...
		if(args.length == 0) exec.runGameTimed(new Human(),new MyGhosts(true),true);
		if(args.length <= 2){
			String command = args[0];
			if(command.equals("debugGhost")){
				int ghost = Integer.parseInt(args[1]);
				boolean[] tmp = new boolean[4];
				tmp[ghost] = true;
				exec.runGameTimed(new Human(), new MyGhosts(true, tmp), true);
			} else if (command.equals("noDebug")){
				exec.runGameTimed(new Human(), new MyGhosts(false), true);
			} else if(command.equals("useAI")){
				String ai = args[1];
				boolean[] tmp = new boolean[4];
				Arrays.fill(tmp, true);
				exec.runGameTimed(new Human(), new MyGhosts(true, tmp, ai+".txt"), true);
			}
		}
